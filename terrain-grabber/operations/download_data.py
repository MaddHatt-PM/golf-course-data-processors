...
def pil_to_cv(pil_image):
    """
    Convert RGB to BGR
    (Legacy helper function from cv2 implementation)
    """
...
def __via_google_satelite(target:LocationPaths, p0:tuple[float, float], p1:tuple[float, float]) -> Path:
    """
    Private function to download satelite imagery through google images
    Image manipulation and saving is handled through cv2 module
    """
...
def download_elevation_for_location(target:LocationPaths, service:services, sample_dist=5) -> Path:
    ...
    points = get_points(*coords, distance=sample_dist, boundry_pts=None)
    output = __via_google_elevation(target, points, target.elevation_csv_path)
    loadingHandler.kill()
    return output
...
def download_elevation(target:LocationPaths, points:tuple[list[float], list[float]], service:services, sample_dist=5, output_path:Path=None, sample_inside_polygon=True) -> Path:
    ...
    if sample_inside_polygon is True:
        coords = target.coordinates()
        points = get_points(*coords, distance=5, boundry_pts=points)
...
def __via_google_elevation(target:LocationPaths, points, output_path:Path) -> Path:
    ...
    if target is not None:
        output_path = target.elevation_csv_path 
...
def get_points(p0, p1, distance=5, boundry_pts=None):
    """
    Given two latitude/longitude coordinates, create a collection of points
    around the perimeter and spaced apart in a 2D area by distance feet.
    """
    geod:Geodesic = Geodesic.WGS84
    lat_line:GeodesicLine = geod.InverseLine( *p0, p0[0], p1[1] )
    lat_line_pts = []

    count = int(math.ceil(lat_line.s13 / distance))
    for i in range(count + 1):
        hor_offset = min(distance * i, lat_line.s13)
    ...
    for lat_pts in lat_line_pts:
        ...
        count = int(math.ceil(long_line.s13 / distance))
        
        for i in range(count + 1):
            ver_offset = min(distance * i, long_line.s13)
