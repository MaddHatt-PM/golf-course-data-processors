...
class PolygonDrawer(object):
    ...
    def on_mouse(self, event, x, y, buttons, user_param): #, holder_list, holder_list_adj):
        ...
        if event == cv2.EVENT_MOUSEMOVE:
            ...
            print("Adding point #%d with position(%d,%d)" % (len(self.holder_list), x, adj_y))
    ...
    def run(self,feature_type,recurse_img):
        ...
        while(not self.done):
            ...
            if (len(self.holder_list) > 0):
                ...
                cv2.polylines(self.canvas, np.array([self.holder_list]), False, FINAL_LINE_COLOR, 1)
                #And  also show what the current segment would look like
                cv2.line(self.canvas, self.holder_list[-1], self.current, WORKING_LINE_COLOR)
            #Update the window
            cv2.imshow(self.window_name, self.canvas)
...
#                fill_index = i
        if (len(self.points[feature_type][working_index]) > 0):
            ...
            cv2.fillPoly(self.featureCanv,np.array([self.holder_list]), FEATURE_COLORS[feature_index])
